    ...
    def predict(self, X):
        ...
        for _ in range(self.fcst_periods):
            ...
            if self.method == 'median':
                y_fcst = np.median(X_temp.iloc[-self.cycle_periods-1:-self.cycle_periods*self.look_back_cycle-2:-self.cycle_periods].values)
            elif self.method == 'mean':
                y_fcst = np.mean(X_temp.iloc[-self.cycle_periods-1:-self.cycle_periods*self.look_back_cycle-2:-self.cycle_periods].values)
            elif self.method == 'drift': # last observation + average change
                X_temp2 = X_temp.iloc[-self.cycle_periods-1:-self.cycle_periods*self.look_back_cycle-2:-self.cycle_periods].sort_index().values.flatten()
                if len(X_temp2) == 1:
                    y_fcst = X_temp2[-1] 
                else:
                    y_fcst = X_temp2[-1] + ( (X_temp2[-1]-X_temp2[0]) / (len(X_temp2)-1) )
            elif self.method == 'ewm':
                X_temp2 = X_temp.iloc[-self.cycle_periods-1:-self.cycle_periods*self.look_back_cycle-2:-self.cycle_periods].sort_index()
                if len(X_temp2) == 1:
