...
def replace_outlier(group, ycol, outlier_profile, min_month, max_month):
...
def handle_outlier(df, outlier_profile, ycol):
    ...
    outlier_grouped = df.groupby(['month', 'time']).apply(lambda group: is_outlier(group, ycol, outlier_profile, min_month, max_month))
    y_grouped = df.groupby(['month', 'time']).apply(lambda group: replace_outlier(group, ycol, outlier_profile, min_month, max_month))

    index_ls = []
    y_ls = []
    outlier_ls = []
    for i in y_grouped.index:
        temp = y_grouped[i]
        for j in range(len(temp)):
            index_ls.append(datetime.datetime(year=i[0].year, month=i[0].month, day=j+1, hour=i[1].hour, minute=i[1].minute))
            y_ls.append(temp[j])
            outlier_ls.append(outlier_grouped[i][j])

    df['new_y'] = pd.Series(data=y_ls, index=index_ls).sort_index().values
    df['outlier'] = pd.Series(data=outlier_ls, index=index_ls).sort_index().values
    
    print(f"Percentage of outlier: {df['outlier'].value_counts()[True]/len(df['outlier'])*100}%")
    plt.rc('figure',figsize=(20,9))
    plt.rc('font',size=12)
    sns.scatterplot(df.index, ycol, data=df, hue='outlier')
...
class Preprocessor(BaseEstimator, TransformerMixin):
    ...
    """
    def __init__(self, resample_freq='30min', na_th='1h', agg='mean', fullday=False, remove_outlier=True, iqr_coeff=1.7):
    ...
    def transform(self, X):
        ...
        ## Replace Outliers
        if self.remove_outlier:
            X = handle_outlier(X, self.outlier_profile, self.ycol)
