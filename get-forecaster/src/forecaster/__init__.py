...
from forecaster.models.linear_model import LinearModel
#from forecaster.models.prophet_model import ProphetModel
from forecaster.models.xgb_model import XGBModel
from forecaster.models.lgb_model import LGBModel
from forecaster.models.sarimax_model import SarimaxModel
from forecaster.models.lstm_model import LSTMModel
from forecaster.models.hybrid_model import HybridModel
from forecaster.utils.model_selection import back_test, randomized_search
...
class AutomatedForecasting(BaseEstimator, RegressorMixin):
    ...
    """
    def __init__(self, freq='30min', horizon='1d', models=['Linear', 'LGB', 'XGB', 'Sarimax', 'LSTM', 'Hybrid'], hybrid_stage1=['LinearTrend'], hybrid_stage2=['Linear', 'LGB', 'XGB', 'LSTM', 'Sarimax'], test_size=0.2, stride='1d', retrain_stride=False, metric='rmse', iteration=50, method='bayesian', early_stopping_steps=10):
    ...
    def fit(self, X, y=None):
        ...
        if self.method == 'bayesian':
            ...
            if model_name == 'Hybrid':
                ...
                print('model1: ', {'model':model1_name, **model1_params})
                print('model2: ', {'model':model2_name, **model2_params})
            else:
                print('best_model: ', {'model':model_name, **hyperparams})
            ...
            if model_name == 'Hybrid':
                model1 = best_res['model1']
                model2 = best_res['model2']
                hyperparams1 = best_res['parameter']['model1_params']
                hyperparams2 = best_res['parameter']['model2_params']
                best_model = HybridModel(freq=self.freq, horizon=self.horizon, model1=model1, model2=model2, model1_params=hyperparams1, model2_params=hyperparams2)
                best_model.fit(X)
                print('===training completed===')
                print('best model: ', model_name)
                print('model1: ', {'model':model1, **hyperparams1})
                print('model2: ', {'model':model2, **hyperparams2})
            
            else: # base models
                hyperparams = best_res['parameter']
                if model_name in ['Linear', 'Sarimax', 'LSTM']:
